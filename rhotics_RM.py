...
def rhotics_export(config, corpus_name, corpus_directory, dialect_code, speakers, vowel_prototypes_path, reset_formants, ignored_speakers=None):
    ...
    with CorpusContext(config) as c:
        ...
        q = q.columns(c.phone.speaker.name.column_name('speaker'),
					  ...
					  #think these names should be correct for additional columns
					  ...
					  #think these may be ok too - these seem to come without user input
					  c.phone.word.transcription.column_name('word_transcription'),
					  c.phone.word.num_syllables.column_name('word_num_syllables'),
					  #but no idea what these are by default as we specify the name in ISCAN - my best guesses:
					  #The below may not actually be crucial - I prob only need syllable final and can calculate this later with values above
					  #c.phone.position_in_syllable.column_name('phone_position_in_syllable'),
					  #c.phone.word.phone_count.column_name('phone_count_in_word'),
					  ...
					  #the below was always here and is obviously necessary!
                      formants_track)
