...
def svlr_export(config, corpus_name, corpus_directory, dialect_code, speakers, vowels, stressed_vowels=None, baseline = False, ignored_speakers=None):
    ## Main duration export function. Collects durational information into query format
    ## and outputs CSV file of measures
    csv_path = os.path.join(base_dir, corpus_name, '{}_svlr.csv'.format(corpus_name))
    ...
    with CorpusContext(config) as c:
        ...
        ## Define the set of vowels be extracted
        vowels = ['i', 'I', 'e', 'E', 'a', 'O', 'V', 'o', '}'              ## normal
                  'i:', 'I:', 'e:', 'E:', 'a:', 'O:', 'V:', 'o:', '}:',    ## stressed
                  'i`', 'i`', 'e`', 'E`', 'a`', 'O`', 'o`', '}`']          ## rhotics
        ...
        q = q.columns(c.sound.lable.column_name('sound_file'),
                      ...
                      c.phone.position.column_name('phone_position'),
        ...
        common.save_performance_benchmark(config, 'svlr_export', time_taken)
...
if __name__ == '__main__':
    ...
    with ensure_local_database_running(corpus_name, port=common.server_port, ip=ip, token=common.load_token()) as params:
        ...
        svlr_export(config, corpus_name, corpus_conf['corpus_directory'], corpus_conf['dialect_code'], corpus_conf['speakers'], corpus_conf['vowel_inventory'], stressed_vowels=stressed_vowels, baseline = baseline, ignored_speakers=ignored_speakers)
        print('Finishing up!')
